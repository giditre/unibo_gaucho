...
for i in {1..3} ; do
  ...
  while [[ "$(curl --write-out %{http_code} http://192.168.10.117:5001/app/FA002 2>/dev/null | tee -a ../gauchotest/allocation_test.log | tee /tmp/last_allocation_test.log | tail -1)" == 20* ]] ; do
